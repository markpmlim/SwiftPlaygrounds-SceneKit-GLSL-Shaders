...
public let kNumTeapotVertices = teapotVertices.count/3
public let kNumTeapotVerticesPerPatch = 16     // 4x4 Bezier patches
public let kNumTeapotPatches = teapotIndices.count/kNumTeapotVerticesPerPatch
public let kNumTeapotIndices = teapotIndices.count

public func createGeometry() -> SCNGeometry? {
    let indicesData = Data(bytes: teapotIndices,
                           count: MemoryLayout<UInt32>.stride * teapotIndices.count)
    let geoElem = SCNGeometryElement(data: indicesData,
                                     primitiveType: SCNGeometryPrimitiveType.point,
                                     primitiveCount: teapotIndices.count,
                                     bytesPerIndex: MemoryLayout<UInt32>.stride)
    let elements = [geoElem]

    let vertexData = Data(bytes: teapotVertices,
                          count: MemoryLayout<Float>.stride * teapotVertices.count)

    let vertexSource = SCNGeometrySource(data: vertexData,
                                         semantic: .vertex,
                                         vectorCount: kNumTeapotVertices,
    ...
    let geo = SCNGeometry(sources: sources,
                          elements: elements)
    return geo
}
